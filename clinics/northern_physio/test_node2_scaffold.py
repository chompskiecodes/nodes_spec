...
# async_capture_context was retired fleet-wide 2026-05-04 (commit e54922a2) — context
# capture is now silent CONTEXT PIGGYBACK onto whichever routing call fires next, not a
# separate tool call. No replacement tool_id is attached for it.
...
def generate_tests(spec: Dict) -> List[Dict]:
    ...
    tests.append({
        ...
        "success_condition": (
            ...
            f'Agent does NOT ask a service question, does NOT call universal_router — the stated service '
            f'(implied_service) is captured silently and piggybacked onto whichever routing call fires next, '
            f'not sent via any tool call this turn. ZERO tool calls is the correct behavior. '
            f'Only fail if universal_router is called or if the billing question is NOT spoken.'
        ),
    ...
    # booking_for defaults to "self" — when caller says "for my husband", it's captured silently
    # (CONTEXT PIGGYBACK, no tool call) as "other", and the agent should ask the OTHER-form
    # billing question directly.
    tests.append({
