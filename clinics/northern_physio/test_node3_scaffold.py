...
Strategy for post-smart_router tests:
  ...
  The Node 3 prompt (gpt-4.1) reads this text and populates its internal state
...
# capture is now silent CONTEXT PIGGYBACK, not a separate tool call. No replacement
# tool_id is attached for it (Node 3's prompt has no capture/piggyback logic anyway —
# booking_for etc. arrive pre-resolved from Node 2 as dynamic variables).
...
SCAFFOLD_LLM       = "gpt-4.1"  # matches production Node 3 LLM (migrated 2026-06-04, was claude-haiku-4-5)
