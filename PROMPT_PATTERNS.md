...
**Node 8 (added 2026-09-21):** the same defect in Node 8's CANCEL/RESCHEDULE ESCAPE (Signal 1, reschedule branch): `nodes/node8_templates/node_8_template.txt`, generated into 31 clinics by `scripts/generate_node8.py` and hand-maintained in `bob_ward_physio` / `totally_well` / `sports_therapy_by_andy`, wrote `call universal_router intent="cancel_intent", reschedule_mode="true", called_number, caller_id. HALT.` It now writes `intent="cancel_intent", called_number, caller_id, payload={"reschedule_mode": "true"}` (Node 2's escape form). Unlike Node 9 / Node 1, a schema-aware Haiku pack DID reproduce it: with the tool described by its real parameters the old line produced a root-level `reschedule_mode` in 12 of 13 answers (the payload empty or holding only the piggyback fields), the new line 0 of 13, because no explicit `payload: {...}` template sits near the line to outrank it (Node 1's Part 2 template does). Literal-following pack: 8 of 8 root -> 0 of 8 (0 of 24 more on the three hand-maintained clinics). The other Node 8 calls (SEND DETAILS, BOOKING SIGNAL, wrap_up, pure cancel, appointment-state query) are unchanged; `smart_voice_agent` calls (`update_contact_detail` `field=`/`value=`, pricing) keep their fields at the root because that tool declares them. Watch item: 2 of 10 new-text answers on a reschedule that also names a practitioner/time dropped those piggyback fields (0 of 7 old-text; p ~ 0.33, not distinguishable from noise) - Node 8's own Signal 0 shows the additive form (`payload: { ... [+ any CONTEXT PIGGYBACK fields] }`) if it ever matters. Local Haiku agents only (Node 8 runs gpt-4.1): text-copy evidence, not a gpt-4.1 result. Contract: `tests/test_node8_payload_scope_contract.py`. Battery: `tests/node_payload_scope/node8_calls.json`.
